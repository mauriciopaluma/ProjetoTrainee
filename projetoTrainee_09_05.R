#install.packages("e1071") # Instala o pacote
#install.packages("dplyr") # Instala o pacote

library(dplyr) # para a tabela das descritivas
library(e1071) # para usar a função kurtosis()
...
## teste ks para temperatura (x1) - Testar a hipotese que os dados da
###  da coluna:"Temperature...C." seguem uma Ditribuição Uniforme 
min_x1 <- min(x1) # 100.0145
max_x1 <- max(x1) # 299.9928

ks.test(x1, "punif", min_x1, max_x1) # p-valor = 0.5679 
# não há indicios contra a hipotese

## teste ks para Pressão (x2) - Testar a hipotese que os dados da
###  da coluna:"Pressão" seguem uma Ditribuição Uniforme 
min_x2 <- min(x2) # 5.003008
max_x2 <- max(x2) # 24.99913

ks.test(x2, "punif", min_x2, max_x2) # p-valor = 0.2287
# não há indicios contra a hipotese


### teste para temperatura x pressão (tentativa)
#ks.test(x3, "pgamma")
library(MASS)
fit <- fitdistr(x3, "gamma")
print(fit)

ks.test(x3, "pgamma", shape = 3.952290e+00, scale = 1.337347e-03 )
# nessa tentativa eu obtive um p valor extremamente baixo
...
shapiro.test(Classificacao_de_qualidade)
shapiro.test(Temperatura)
shapiro.test(Pressao)
shapiro.test(TemperaturaxPressao)
shapiro.test(Metrica_de_fusao_de_materiais)
shapiro.test(Metrica_de_transformacao_de_materiais)

# 2º - Passo Teste de correlação e Matriz de Correlação -------------------------------------------------------------

#Correlação de kendall
cor.test(Classificacao_de_qualidade,Temperatura, method = "kendall") # -0.9924301  - Correlação significativa e negativa

cor.test(Classificacao_de_qualidade, Pressao, method = "kendall") # 0.01665777 - Não há correlação significativa, p-valor alto 

cor.test(Classificacao_de_qualidade, TemperaturaxPressao, method = "kendall") # -0.3881595  - Correlação significativa e negativa

cor.test(Classificacao_de_qualidade, Metrica_de_fusao_de_materiais, method = "kendall") # -0.8806713  - Correlação significativa e negativa

cor.test(Classificacao_de_qualidade, Metrica_de_transformacao_de_materiais, method = "kendall") # -0.9924414 - Correlação significativa e negativa
